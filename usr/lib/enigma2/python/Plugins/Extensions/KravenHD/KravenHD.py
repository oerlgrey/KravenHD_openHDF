...
from Components.config import config, configfile, getConfigListEntry, ConfigSubsection, ConfigSelection, ConfigText, ConfigClock, ConfigSlider
...
import gettext, os, time, subprocess, requests
from enigma import ePicLoad, getDesktop, eConsoleAppContainer, eTimer
from Tools.Directories import fileExists, resolveFilename, SCOPE_LANGUAGE, SCOPE_PLUGINS, defaultPaths
...
"""
	...
	def updateMylist(self):
		...
		for i in range(emptyLines+1):
			list.append(getConfigListEntry("     ", config.plugins.KravenHD.EmptyLine, "     "))
		...
		if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x1", "infobar-style-x2", "infobar-style-x4", "infobar-style-z1", "infobar-style-zz1", "infobar-style-zzz1"):
			...
			emptyLines+=5
		if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x4", "infobar-style-zz1", "infobar-style-zzz1"):
			...
			emptyLines+=2
		if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x1", "infobar-style-x2", "infobar-style-x3", "infobar-style-x4", "infobar-style-z1", "infobar-style-z2", "infobar-style-zz1"):
		...
		if self.InternetAvailable:
			...
			if isPluginInstalled("OAWeather"):
				list.append(getConfigListEntry(_("Use OAWeather"), config.plugins.KravenHD.UseOAWeather, _("Choose if you want to use the OAWeather plugin.")))
			else:
				emptyLines+=1
			if isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
				emptyLines+=5
			else:
				list.append(getConfigListEntry(_("Search option"), config.plugins.KravenHD.searchby, _("Choose from different options to enter your settings.\nThen press the red button to search for the coordinates.")))
				if config.plugins.KravenHD.searchby.value == "location":
					list.append(getConfigListEntry(_("Location "), config.plugins.KravenHD.cityname, _("Enter your location.\nPress OK to use the virtual keyboard.\nThen press the red button to search for the coordinates.")))
				else:
					emptyLines+=1
		...
		if self.InternetAvailable:
			if isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
				list.append(getConfigListEntry(_("Clock-Style"), config.plugins.KravenHD.ClockStyleOAWeather, _("Choose from different options to show the clock in the infobar.")))
				self.actClockstyle=config.plugins.KravenHD.ClockStyleOAWeather.value
			else:
		...
		if config.plugins.KravenHD.InfobarStyle.value == "infobar-style-x1" and not config.plugins.KravenHD.ECMVisible.value == "none":
			...
			list.append(getConfigListEntry(_("Show 'free to air'"), config.plugins.KravenHD.FTA, _("Choose whether 'free to air' is displayed or not for unencrypted channels.")))
			list.append(getConfigListEntry(_("ECM-Font"), config.plugins.KravenHD.ECMFontList, _("Choose the font color of the ECM information. Press OK to define your own RGB color.")))
		elif config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-x4", "infobar-style-zz1", "infobar-style-zz2", "infobar-style-zz3", "infobar-style-zzz1") and not config.plugins.KravenHD.ECMVisible.value == "none":
		...
		list.append(getConfigListEntry(_("Sherlock"), config.plugins.KravenHD.Sherlock, _("Choose whether Sherlock should be displayed as original or modified.")))
		...
		if not config.plugins.KravenHD.ChannelSelectionStyle.value in ("channelselection-style-minitv", "channelselection-style-minitv4", "channelselection-style-nobile-minitv", "channelselection-style-minitv-picon"):
		...
		if config.plugins.KravenHD.ChannelSelectionStyle.value in ("channelselection-style-nobile", "channelselection-style-nobile2", "channelselection-style-nobile-minitv"):
			...
			list.append(getConfigListEntry(_("EPG Fontsize"), config.plugins.KravenHD.ChannelSelectionEPGSize2, _("Choose the font size of event description, EPG list and primetime.")))
		...
		list.append(getConfigListEntry(_("EPG-List Font"), config.plugins.KravenHD.GMEFontColorList, _("Choose the font color in the EPG-List. Press OK to define your own RGB color.")))
		list.append(getConfigListEntry(_("EPG-List Background"), config.plugins.KravenHD.GMEOtherColorList, _("Choose the background color in the EPG-List. Press OK to define your own RGB color.")))
		list.append(getConfigListEntry(_("EPG-List selected Font"), config.plugins.KravenHD.GMESelectedFontColorList, _("Choose the font color of the selected event in the EPG-List. Press OK to define your own RGB color.")))
		list.append(getConfigListEntry(_("EPG-List selected Background"), config.plugins.KravenHD.GMESelectedColorList, _("Choose the background color of the selected event in the EPG-List. Press OK to define your own RGB color.")))
		list.append(getConfigListEntry(_("EPG-List current Background"), config.plugins.KravenHD.GMECurrentColorList, _("Choose the background color of current running events in the EPG-List. Press OK to define your own RGB color.")))
		for i in range(emptyLines+1):
			list.append(getConfigListEntry("     ", config.plugins.KravenHD.EmptyLine, "     "))
		...
		if self.InternetAvailable:
			if isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
				list.append(getConfigListEntry(_("Clock"), config.plugins.KravenHD.PlayerClockOAWeather, _("Choose from different options to show the clock in the players.")))
				self.actPlayerstyle=config.plugins.KravenHD.PlayerClockOAWeather.value
				emptyLines+=1
			else:
				...
				self.actPlayerstyle=config.plugins.KravenHD.PlayerClock.value
				if self.actPlayerstyle == "player-android":
			...
			list.append(getConfigListEntry(_("Clock"), config.plugins.KravenHD.PlayerClockNoInternet, _("Choose from different options to show the clock in the players.")))
			self.actPlayerstyle=config.plugins.KravenHD.PlayerClockNoInternet.value
			emptyLines+=1
		...
		if config.plugins.KravenHD.GMECurrentColorList.value == "self":
			config.plugins.KravenHD.GMECurrentColor.value = config.plugins.KravenHD.GMECurrentColorSelf.value
		else:
			config.plugins.KravenHD.GMECurrentColor.value = config.plugins.KravenHD.GMECurrentColorList.value
		if config.plugins.KravenHD.GMESelectedColorList.value == "self":
			config.plugins.KravenHD.GMESelectedColor.value = config.plugins.KravenHD.GMESelectedColorSelf.value
		else:
			config.plugins.KravenHD.GMESelectedColor.value = config.plugins.KravenHD.GMESelectedColorList.value
		if config.plugins.KravenHD.GMEOtherColorList.value == "self":
			config.plugins.KravenHD.GMEOtherColor.value = config.plugins.KravenHD.GMEOtherColorSelf.value
		else:
			config.plugins.KravenHD.GMEOtherColor.value = config.plugins.KravenHD.GMEOtherColorList.value
		if config.plugins.KravenHD.GMEFontColorList.value == "self":
			config.plugins.KravenHD.GMEFontColor.value = config.plugins.KravenHD.GMEFontColorSelf.value
		else:
			config.plugins.KravenHD.GMEFontColor.value = config.plugins.KravenHD.GMEFontColorList.value
		if config.plugins.KravenHD.GMESelectedFontColorList.value == "self":
			config.plugins.KravenHD.GMESelectedFontColor.value = config.plugins.KravenHD.GMESelectedFontColorSelf.value
		else:
			config.plugins.KravenHD.GMESelectedFontColor.value = config.plugins.KravenHD.GMESelectedFontColorList.value
	...
	def showPreview(self):
		...
		if option.value == "none":
		...
		elif option in (config.plugins.KravenHD.searchby, config.plugins.KravenHD.cityname):
			if config.plugins.KravenHD.cityfound.value != "" and config.plugins.KravenHD.latitude.value != "" and config.plugins.KravenHD.longitude.value != "":
				self.showText(30, str(config.plugins.KravenHD.cityfound.value) + "\n" + "lat: " + str(config.plugins.KravenHD.latitude.value) + "\n" + "lon: " + str(config.plugins.KravenHD.longitude.value))
			else:
				self.showText(30, _("No location set."))
			...
			elif option.value == "ShortHops":
				self.showText(17, "CAID - Time - Hops")
		...
		elif option == config.plugins.KravenHD.UseOAWeather:
			if option.value == "on":
				self.showText(62, _("On"))
			else:
				self.showText(62, _("Off"))
		...
		elif option == config.plugins.KravenHD.GMECurrentColorList:
			self.showGradient(config.plugins.KravenHD.GMECurrentColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMECurrentColor.value))
		elif option == config.plugins.KravenHD.GMESelectedColorList:
			self.showGradient(config.plugins.KravenHD.GMESelectedColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMESelectedColor.value))
		elif option == config.plugins.KravenHD.GMEOtherColorList:
			self.showGradient(config.plugins.KravenHD.GMEOtherColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMEOtherColor.value))
		elif option == config.plugins.KravenHD.GMEFontColorList:
			self.showColor(self.hexRGB(config.plugins.KravenHD.GMEFontColor.value))
		elif option == config.plugins.KravenHD.GMESelectedFontColorList:
			self.showColor(self.hexRGB(config.plugins.KravenHD.GMESelectedFontColor.value))
			...
			if option.value == "0":
				...
				self.showText(36, _("Infobar & \nSecondInfobar"))
		elif option == config.plugins.KravenHD.Sherlock:
			if option.value == "sherlock_org":
				self.showText(36, _("Sherlock original"))
			elif option.value == "sherlock_mod":
				self.showText(36, _("Sherlock modified"))
		else:
			self["helperimage"].show()
	...
	def showRedText(self):
		option = self["config"].getCurrent()[1]
		if option in (config.plugins.KravenHD.searchby, config.plugins.KravenHD.cityname):
			self["key_red"].text = _("Find coordinates")
		else:
	...
	def categoryDown(self):
		...
		if not config.plugins.KravenHD.IBStyle.value == "box":
			if (144 <= position <= 147): # antialiasing
				self["config"].instance.moveSelectionTo(141)
		self.mylist()
	...
	def redbutton(self):
		...
		if option in (config.plugins.KravenHD.searchby, config.plugins.KravenHD.cityname):
			if self.InternetAvailable:
				...
				self.session.open(MessageBox, _("You have no internet connection.\nPlease solve the problem."), MessageBox.TYPE_INFO, timeout=8)
		else:
			self.faq()
	...
	def getCityByIP(self, failed):
		...
		try:
			...
			if data['status'] == 'success':
				...
				lon = data['lon']
				if failed == "fallback":
					config.plugins.KravenHD.cityname.value = ""
					config.plugins.KravenHD.cityname.save()
		...
		config.plugins.KravenHD.cityfound.value = str(city)
		config.plugins.KravenHD.cityfound.save()
		config.plugins.KravenHD.latitude.value = str(lat)
		config.plugins.KravenHD.latitude.save()
		config.plugins.KravenHD.longitude.value = str(lon)
		config.plugins.KravenHD.longitude.save()
		self.showPreview()
	...
	def BrowserCallBack(self, callback):
		try:
			if callback:
				posterpath = os.path.join(callback, "poster")
	...
	def keyOK(self):
		...
		if option == config.plugins.KravenHD.BackgroundListColor:
			...
			optionislistcolor=True
			self.actSelfColorSelection = config.plugins.KravenHD.GMECurrentColorSelf
		elif option == config.plugins.KravenHD.GMESelectedColorList:
			optionislistcolor=True
			self.actSelfColorSelection = config.plugins.KravenHD.GMESelectedColorSelf
		elif option == config.plugins.KravenHD.GMEOtherColorList:
			optionislistcolor=True
			self.actSelfColorSelection = config.plugins.KravenHD.GMEOtherColorSelf
		elif option == config.plugins.KravenHD.GMEFontColorList:
			optionislistcolor=True
			self.actSelfColorSelection = config.plugins.KravenHD.GMEFontColorSelf
		elif option == config.plugins.KravenHD.GMESelectedFontColorList:
			optionislistcolor=True
			self.actSelfColorSelection = config.plugins.KravenHD.GMESelectedFontColorSelf
		...
		elif option == config.plugins.KravenHD.cityname:
	...
	def save(self, answer=True):
		...
		if self.actMenustyle in ("logo", "metrix-icons", "empty"):
		...
		elif self.actMenustyle == "empty":
			self.skinSearchAndReplace.append(['<!-- Logo -->', '<panel name="Logo5"/>'])
			self.skinSearchAndReplace.append(['<!-- Metrix-Icons -->', '<panel name="Logo5"/>'])
		else:
		...
		if config.plugins.KravenHD.IBStyle.value == "box":
			...
			if config.plugins.KravenHD.InfobarBoxColor.value == "gradient":
				...
				if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-x2", "infobar-style-x3"):
					...
					self.skinSearchAndReplace.append(['<!-- Infobar ibar -->', '<panel name="infobar-style-x4-zz1-box2"/>'])
				...
				if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-x2", "infobar-style-x3"):
					...
					self.skinSearchAndReplace.append(['<!-- Infobar ibar -->', '<panel name="infobar-style-x4-zz1-texture"/>'])
				elif config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x1", "infobar-style-zzz1"):
				...
				if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-x2", "infobar-style-x3"):
					...
					self.skinSearchAndReplace.append(['<!-- Infobar ibar -->', '<panel name="infobar-style-x4-zz1-box"/>'])
				elif config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x1", "infobar-style-zzz1"):
			...
			if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-x2", "infobar-style-x3"):
				...
				self.skinSearchAndReplace.append(['<!-- Infobar ibar -->', '<panel name="infobar-style-x4-zz1-gradient"/>'])
			elif config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x1", "infobar-style-zzz1"):
		...
		self.skinSearchAndReplace.append(['name="KravenGMEFont" value="#00ffffff', 'name="KravenGMEFont" value="#00' + config.plugins.KravenHD.GMEFontColor.value])
		self.skinSearchAndReplace.append(['name="KravenGMESelFg" value="#00ffffff', 'name="KravenGMESelFg" value="#00' + config.plugins.KravenHD.GMESelectedFontColor.value])
		self.skinSearchAndReplace.append(['name="KravenGMESelBg" value="#00389416', 'name="KravenGMESelBg" value="#00' + config.plugins.KravenHD.GMESelectedColor.value])
		self.skinSearchAndReplace.append(['name="KravenGMENowFg" value="#00F0A30A', 'name="KravenGMENowFg" value="#00' + config.plugins.KravenHD.GMEFontColor.value])
		self.skinSearchAndReplace.append(['name="KravenGMENowBg" value="#000064c7', 'name="KravenGMENowBg" value="#00' + config.plugins.KravenHD.GMECurrentColor.value])
		self.skinSearchAndReplace.append(['name="KravenGMEBg" value="#003F3F3F', 'name="KravenGMEBg" value="#00' + config.plugins.KravenHD.GMEOtherColor.value])
		...
		if config.plugins.KravenHD.SkinResolution.value == "hd":
			if config.plugins.KravenHD.IBFontSize.value == "small":
				...
				self.skinSearchAndReplace.append(['font="Regular;30" position="427,553" size="592,38"', 'font="Regular;22" position="427,560" size="592,27"']) # X4 now
				self.skinSearchAndReplace.append(['font="Regular;30" position="427,643" size="485,38"', 'font="Regular;22" position="427,650" size="485,27"']) # X4 next
				...
				self.skinSearchAndReplace.append(['font="Regular;30" position="427,553" size="592,38"', 'font="Regular;26" position="427,556" size="592,33"']) # X4 now
				self.skinSearchAndReplace.append(['font="Regular;30" position="427,643" size="485,38"', 'font="Regular;26" position="427,646" size="485,33"']) # X4 next
			...
			if config.plugins.KravenHD.IBFontSize.value == "small":
				...
				self.skinSearchAndReplace.append(['font="Regular;45" position="640,830" size="888,55"', 'font="Regular;33" position="640,839" size="888,42"']) # X4 now
				self.skinSearchAndReplace.append(['font="Regular;45" position="640,965" size="727,55"', 'font="Regular;33" position="640,974" size="727,42"']) # X4 next
				...
				self.skinSearchAndReplace.append(['font="Regular;45" position="640,830" size="888,55"', 'font="Regular;39" position="640,833" size="888,49"']) # X4 now
				self.skinSearchAndReplace.append(['font="Regular;45" position="640,965" size="727,55"', 'font="Regular;39" position="640,968" size="727,49"']) # X4 next
		...
		if config.plugins.KravenHD.ChannelSelectionStyle.value in ("channelselection-style-nobile", "channelselection-style-nobile2", "channelselection-style-nobile-minitv"):
			if config.plugins.KravenHD.Primetimeavailable.value == "primetime-on" and config.plugins.KravenHD.ChannelSelectionEPGSize1.value == "big":
				self.skinSearchAndReplace.append(['<panel name="' + config.plugins.KravenHD.ChannelSelectionStyle.value + '-small"/>', '<panel name="' + config.plugins.KravenHD.ChannelSelectionStyle.value + '-big-prime"/>'])
				...
				self.skinSearchAndReplace.append(['<panel name="' + config.plugins.KravenHD.ChannelSelectionStyle.value + '-small"/>', '<panel name="' + config.plugins.KravenHD.ChannelSelectionStyle.value + '-big"/>'])
				...
				self.skinSearchAndReplace.append(['<panel name="' + config.plugins.KravenHD.ChannelSelectionStyle.value + '-small"/>', '<panel name="' + config.plugins.KravenHD.ChannelSelectionStyle.value + '-small-prime"/>'])
		else:
		...
		if config.plugins.KravenHD.IBColor.value == "only-infobar":
			if config.plugins.KravenHD.IconStyle2.value == "icons-dark2":
				...
				self.skinSearchAndReplace.append(["sherlock-icons", "icons-dark"])
				...
				self.skinSearchAndReplace.append(["sherlock-icons", "icons-light"])
		...
		if config.plugins.KravenHD.SkinResolution.value == "hd":
			if config.plugins.KravenHD.WeatherView.value == "meteo":
				...
				self.skinSearchAndReplace.append([',Icon</convert>', ',MeteoFont</convert>'])
		else:
			if config.plugins.KravenHD.WeatherView.value == "meteo":
		...
		if not config.plugins.KravenHD.Progress.value == "progress":
			...
			self.skinSearchAndReplace.append([' pixmap="KravenHD/progress/progress270_2.png"', " "])
		...
		if config.plugins.KravenHD.IBProgressBackgroundList.value == "none":
			...
			self.makeProgressBackground(769, config.plugins.KravenHD.IBProgressBackground.value, "progress_bg_ts") # x1, x2, x3, x4, z1, z2, timeshift
		...
		if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x1"):
			...
			self.appendSkinFile(self.data + 'templates-infobar-style-x4-zz1-zzz1.xml')
		elif config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-zz2", "infobar-style-zz3"):
		...
		### Infobar Clock
		if config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-nopicon", "infobar-style-x1"):
			if self.actClockstyle == "clock-weather" and isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
				self.skinSearchAndReplace.append(['<!-- Infobar clockstyle -->', '<panel name="' + config.plugins.KravenHD.InfobarStyle.value + '-clock-weather_OAWeather"/>'])
			else:
			...
			if self.actClockstyle == "clock-weather" and isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
				self.skinSearchAndReplace.append(['<!-- Infobar clockstyle -->', '<panel name="infobar-style-x2-x3-z1-z2-clock-weather_OAWeather"/>'])
			else:
				self.skinSearchAndReplace.append(['<!-- Infobar clockstyle -->', '<panel name="infobar-style-x2-x3-z1-z2-' + self.actClockstyle + '"/>'])
		elif config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-zz2", "infobar-style-zz3"):
			if self.actClockstyle == "clock-weather" and isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
				self.skinSearchAndReplace.append(['<!-- Infobar clockstyle -->', '<panel name="infobar-style-zz2-zz3-clock-weather_OAWeather"/>'])
			else:
				self.skinSearchAndReplace.append(['<!-- Infobar clockstyle -->', '<panel name="infobar-style-zz2-zz3-' + self.actClockstyle + '"/>'])
		elif config.plugins.KravenHD.InfobarStyle.value in ("infobar-style-x4", "infobar-style-zz1", "infobar-style-zzz1"):
			if self.actClockstyle == "clock-weather" and isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
				self.skinSearchAndReplace.append(['<!-- Infobar clockstyle -->', '<panel name="infobar-style-x4-zz1-zzz1-clock-weather_OAWeather"/>'])
			else:
				self.skinSearchAndReplace.append(['<!-- Infobar clockstyle -->', '<panel name="infobar-style-x4-zz1-zzz1-' + self.actClockstyle + '"/>'])
		...
		if config.plugins.KravenHD.InfobarStyle.value == "infobar-style-nopicon" and not config.plugins.KravenHD.InfobarChannelName.value == "none":
			...
			self.skinSearchAndReplace.append(['<!-- Infobar channelname -->', '<panel name="infobar-style-x4-zz1-' + config.plugins.KravenHD.InfobarChannelName.value + '"/>'])
		...
		if config.plugins.KravenHD.InfobarStyle.value == "infobar-style-nopicon":
			...
			self.skinSearchAndReplace.append(['<!-- Infobar icons -->', '<panel name="infobar-style-x4-zz1-zzz1-icons"/>'])
		...
		### Players clockstyle
		if self.actPlayerstyle == "player-weather" and isPluginInstalled("OAWeather") and config.plugins.KravenHD.UseOAWeather.value == "on":
			self.skinSearchAndReplace.append(['<!-- Player clockstyle -->', '<panel name="player-weather_OAWeather"/>'])
		else:
			self.skinSearchAndReplace.append(['<!-- Player clockstyle -->', '<panel name="' + self.actPlayerstyle + '"/>'])
		...
		if config.plugins.KravenHD.GraphicalEPG.value == "text":
			config.epgselection.graph_type_mode.value = "text"
		...
		### Sherlock
		self.appendSkinFile(self.data + config.plugins.KravenHD.Sherlock.value + ".xml")
		...
		### skin-user
		if fileExists(self.data + "skin-user.xml"):
		...
		self.makeGraphEPGpng("CurrentEvent", config.plugins.KravenHD.GMECurrentColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMECurrentColor.value))
		self.makeGraphEPGpng("CurrentService", config.plugins.KravenHD.GMECurrentColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMECurrentColor.value))
		self.makeGraphEPGpng("OtherEvent", config.plugins.KravenHD.GMEOtherColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMEOtherColor.value))
		self.makeGraphEPGpng("OtherService", config.plugins.KravenHD.GMEOtherColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMEOtherColor.value))
		self.makeGraphEPGpng("SelectedCurrentEvent", config.plugins.KravenHD.GMESelectedColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMESelectedColor.value))
		self.makeGraphEPGpng("SelectedEvent", config.plugins.KravenHD.GMESelectedColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMESelectedColor.value))
		self.makeGraphEPGpng("SelectedRecordEvent", config.plugins.KravenHD.GMESelectedColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMESelectedColor.value))
		self.makeGraphEPGpng("TimeLineTime", config.plugins.KravenHD.GMEOtherColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMEOtherColor.value))
		self.makeGraphEPGpng("TimeLineDate", config.plugins.KravenHD.GMEOtherColor.value, self.calcSecondcolor(config.plugins.KravenHD.GMEOtherColor.value))
		self.makeGraphEPGpng("RecordEvent", "E51400", "330400")
		self.makeGraphEPGpng("RecordingEvent", "E51400", "330400")
	...
	def getTuners(self):
		...
		tuners = len(nimmanager.nim_slots)
		if tuners is 1:
			return "1-tuner"
		elif tuners is 2:
			return "2-tuner"
	...
	def getUserMenuIconsAvailable(self):
		...
		if os.path.exists(userpath) and any(File.endswith(".png") for File in os.listdir(userpath)):
			return True
		else:
			return False
	...
	def makeGraphEPGpng(self, name, color1, color2):
		width = int(8)
		height = int(100)
